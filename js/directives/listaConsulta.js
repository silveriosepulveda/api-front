                    ...
                    angular.forEach(parametros.listaConsulta, function (val, key) {
                        ...
                        if (val.tipo != "oculto") {
                            if (val.tipo == "caixaSelecao") {
                                ...
                                htmlCamposLista += `<div class="col-xs-12 form-group form-group-modern ${classesDivItemConsulta} ${tamanho.join(
                                    " "
                                )}" ${ngClasses} ${atributosDivCampoConsulta.join(" ")}>
    ...
    .directive("contadorFiltro", ($compile) => {
        return {
            ...
            link: (scope, elem, attr) => {
                ...
                scope.$watchGroup(
                    ["listaConsultaCompleta.length", "listaConsulta.length", "listaConsultaVisivel.length", "filtroResultado"],
                    atualizarContador
                );
