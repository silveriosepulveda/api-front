directivesPadrao.directive('autoCompleta', ['$rootScope', '$parse', 'APIServ', 'EGFuncoes', '$timeout', function ($rootScope, $parse, APIServ, EGFuncoes, $timeout) {
    return {
        restrict: "A",
        link: function (scope, elem, attr) {
            ...
            // Flags para evitar execuções recursivas (uma por função)
            var funcoesExecutando = {};
            ...
            // Função auxiliar para encontrar o scope correto para atribuir valores
            // Tenta encontrar o scope do formulário ou elemento (dentro ou fora do modal)
            function encontrarScopeParaAtribuir(expr, elemento) {
                if (!expr) {
                    return scope;
                }
                
                var elementoParaBuscar = null;
                
                // Se foi fornecido um elemento, usar ele, senão usar o elemento da diretiva
                if (elemento) {
                    try {
                        elementoParaBuscar = $(elemento);
                    } catch (e) {
                        elementoParaBuscar = elem;
                    }
                } else {
                    elementoParaBuscar = elem;
                }
                
                // Extrair a primeira parte da expressão (objeto raiz)
                // Ex: "torneio['titulo_temporada']" -> "torneio"
                var primeiraParte = expr.split(/[\.\[\']/)[0];
                
                // Estratégia 1: Procurar no formulário pai (dentro ou fora de modais)
                try {
                    var formElement = elementoParaBuscar.closest('form');
                    if (formElement && formElement.length > 0) {
                        var formScope = angular.element(formElement[0]).scope();
                        if (formScope) {
                            // Procurar o scope que contém o objeto raiz, subindo na hierarquia
                            var scopeAtual = formScope;
                            var nivel = 0;
                            var scopesVisitados = [];
                            while (scopeAtual && nivel < 20) {
                                nivel++;
                                
                                // Evitar loops
                                if (scopeAtual.$id && scopesVisitados.indexOf(scopeAtual.$id) !== -1) {
                                    break;
                                }
                                if (scopeAtual.$id) {
                                    scopesVisitados.push(scopeAtual.$id);
                                }
                                
                                // Verificar se este scope contém a propriedade raiz
                                try {
                                    var parseFnRaiz = $parse(primeiraParte);
                                    var valorRaiz = parseFnRaiz(scopeAtual);
                                    if (valorRaiz !== undefined) {
                                        return scopeAtual;
                                    }
                                } catch (e) {
                                ...
                                // Se não encontrou, subir para o scope pai
                                if (scopeAtual.$parent) {
                            ...
                            // Se não encontrou o objeto raiz, usar o scope do formulário mesmo assim
                            return formScope;
                        }
                    }
                } catch (e) {
                ...
                // Estratégia 2: Tentar obter scope do elemento atual e procurar o objeto raiz
                try {
                    if (elementoParaBuscar && elementoParaBuscar.length > 0) {
                        var scopeElemento = angular.element(elementoParaBuscar[0]).scope();
                        if (scopeElemento) {
                            // Procurar o scope que contém o objeto raiz, subindo na hierarquia
                            var scopeAtual2 = scopeElemento;
                            var nivel2 = 0;
                            var scopesVisitados2 = [];
                            while (scopeAtual2 && nivel2 < 20) {
                                nivel2++;
                                
                                // Evitar loops
                                if (scopeAtual2.$id && scopesVisitados2.indexOf(scopeAtual2.$id) !== -1) {
                                    break;
                                }
                                if (scopeAtual2.$id) {
                                    scopesVisitados2.push(scopeAtual2.$id);
                                }
                                
                                // Verificar se este scope contém a propriedade raiz
                                try {
                                    var parseFnRaiz2 = $parse(primeiraParte);
                                    var valorRaiz2 = parseFnRaiz2(scopeAtual2);
                                    if (valorRaiz2 !== undefined) {
                                        return scopeAtual2;
                                    }
                                } catch (e) {
                                    // Continuar procurando
                                ...
                                // Se não encontrou, subir para o scope pai
                                if (scopeAtual2.$parent) {
                                    scopeAtual2 = scopeAtual2.$parent;
                                } else {
                                    break;
                                }
                            }
                            
                            // Se não encontrou o objeto raiz, usar o scope do elemento mesmo assim
                            return scopeElemento;
                        }
                    }
                } catch (e) {
                    // Ignorar erro
                ...
                // Fallback: usar o scope original
                return scope;
            }

            // Função auxiliar para atribuir valores dinâmicos de forma segura
            function setScopeValue(expr, value, elemento) {
                try {
                    var scopeParaAtribuir = encontrarScopeParaAtribuir(expr, elemento);
                    $parse(expr).assign(scopeParaAtribuir, value);
                } catch (e) {
                    console.warn('⚠️ [autoCompleta] Erro ao atribuir valor:', expr, e);
                }
            }

            // Função auxiliar para encontrar e executar funções no scope correto
            // Procura no scope atual e sobe na hierarquia até encontrar
            function encontrarEExecutarFuncaoNoScope(nomeFuncao, scopeAtual, parametros) {
                if (!nomeFuncao || !nomeFuncao.trim()) {
                    return null;
                }
                
                // Extrair apenas o nome da função (sem parâmetros)
                // Ex: "limparPassaro(novoResultado)" -> "limparPassaro"
                var nomeFuncaoSemParenteses = nomeFuncao.trim().replace(/\([^)]*\)/g, '').replace(/\(\)/g, '');
                
                // Evitar execuções recursivas da mesma função
                if (funcoesExecutando[nomeFuncaoSemParenteses]) {
                    console.log('⚠️ [autoCompleta] Função já está sendo executada, evitando recursão:', nomeFuncaoSemParenteses);
                    return null;
                }
                
                var scopeParaBuscar = scopeAtual || scope;
                var maxNiveis = 30; // Aumentar limite para garantir que encontra
                var nivel = 0;
                var scopesVisitados = [];
                
                console.log('🔍 [autoCompleta] Procurando função:', nomeFuncaoSemParenteses, 'no scope atual');
                
                // Subir na hierarquia de scopes até encontrar a função
                while (scopeParaBuscar && nivel < maxNiveis) {
                    nivel++;
                    
                    // Evitar loops infinitos verificando se já visitamos este scope
                    if (scopeParaBuscar.$id && scopesVisitados.indexOf(scopeParaBuscar.$id) !== -1) {
                        console.log('⚠️ [autoCompleta] Scope já visitado, parando busca. Scope ID:', scopeParaBuscar.$id);
                        break;
                    }
                    if (scopeParaBuscar.$id) {
                        scopesVisitados.push(scopeParaBuscar.$id);
                    }
                    
                    // Debug: mostrar qual scope está sendo verificado
                    if (nivel <= 5) { // Log apenas nos primeiros 5 níveis para não poluir
                        console.log('🔍 [autoCompleta] Verificando scope nível', nivel, 'ID:', scopeParaBuscar.$id);
                    }
                    
                    // Tentar acesso direto ao scope primeiro (mais rápido)
                    if (scopeParaBuscar[nomeFuncaoSemParenteses] && typeof scopeParaBuscar[nomeFuncaoSemParenteses] === 'function') {
                        console.log('✅ [autoCompleta] Função encontrada no scope nível', nivel, 'ID:', scopeParaBuscar.$id);
                        try {
                            // Marcar função como executando
                            funcoesExecutando[nomeFuncaoSemParenteses] = true;
                            
                            var resultado;
                            if (parametros !== undefined) {
                                resultado = scopeParaBuscar[nomeFuncaoSemParenteses].apply(scopeParaBuscar, Array.isArray(parametros) ? parametros : [parametros]);
                            } else {
                                resultado = scopeParaBuscar[nomeFuncaoSemParenteses].call(scopeParaBuscar);
                            }
                            console.log('✅ [autoCompleta] Função executada com sucesso:', nomeFuncaoSemParenteses);
                            
                            // Resetar flag após um delay para evitar loops causados por eventos disparados pela função
                            $timeout(function() {
                                funcoesExecutando[nomeFuncaoSemParenteses] = false;
                            }, 300);
                            
                            return resultado;
                        } catch (e) {
                            console.error('❌ [autoCompleta] Erro ao executar função:', nomeFuncaoSemParenteses, e);
                    ...
                    // Tentar usar $parse para avaliar a função
                    try {
                        var parseFn = $parse(nomeFuncaoSemParenteses);
                        var funcao = parseFn(scopeParaBuscar);
                        
                        if (typeof funcao === 'function') {
                            console.log('✅ [autoCompleta] Função encontrada via $parse no scope nível', nivel, 'ID:', scopeParaBuscar.$id);
                            try {
                                // Marcar função como executando
                                funcoesExecutando[nomeFuncaoSemParenteses] = true;
                                ...
                                var resultado;
                                if (parametros !== undefined) {
                                    resultado = funcao.apply(scopeParaBuscar, Array.isArray(parametros) ? parametros : [parametros]);
                                } else {
                                    resultado = funcao.call(scopeParaBuscar);
                                }
                                console.log('✅ [autoCompleta] Função executada com sucesso (via $parse):', nomeFuncaoSemParenteses);
                                ...
                                console.error('❌ [autoCompleta] Erro ao executar função (parse):', nomeFuncaoSemParenteses, e);
                                // Resetar flag em caso de erro também
                                funcoesExecutando[nomeFuncaoSemParenteses] = false;
                                return null;
                            }
                        ...
                        // Continuar procurando no scope pai
                    }
                    
                    // Subir para o scope pai, mas não parar no $rootScope imediatamente
                    if (scopeParaBuscar.$parent) {
                        scopeParaBuscar = scopeParaBuscar.$parent;
                    } else {
                        // Se chegou ao $rootScope, verificar nele também antes de parar
                        if (scopeParaBuscar === $rootScope) {
                            // Já estamos no $rootScope, tentar uma última vez
                            if ($rootScope[nomeFuncaoSemParenteses] && typeof $rootScope[nomeFuncaoSemParenteses] === 'function') {
                                console.log('✅ [autoCompleta] Função encontrada no $rootScope');
                                try {
                                    // Marcar função como executando
                                    funcoesExecutando[nomeFuncaoSemParenteses] = true;
                                    ...
                                    var resultado;
                                    if (parametros !== undefined) {
                                        resultado = $rootScope[nomeFuncaoSemParenteses].apply($rootScope, Array.isArray(parametros) ? parametros : [parametros]);
                                    } else {
                                        resultado = $rootScope[nomeFuncaoSemParenteses].call($rootScope);
                                    }
                                    ...
                                    console.error('❌ [autoCompleta] Erro ao executar função no $rootScope:', nomeFuncaoSemParenteses, e);
                                    funcoesExecutando[nomeFuncaoSemParenteses] = false;
                                }
                            }
                        }
                ...
                // Estratégia 2: Procurar no formulário pai (fora de modais)
                try {
                    var formElement = elem.closest('form');
                    if (formElement && formElement.length > 0) {
                        var formScope = angular.element(formElement[0]).scope();
                        if (formScope && formScope.$id !== scope.$id) {
                            console.log('🔍 [autoCompleta] Tentando procurar função no scope do formulário');
                            var formScopeAtual = formScope;
                            var nivelForm = 0;
                            var scopesFormVisitados = [];
                            while (formScopeAtual && nivelForm < 15) {
                                nivelForm++;
                                if (formScopeAtual.$id && scopesFormVisitados.indexOf(formScopeAtual.$id) !== -1) {
                                    break;
                                }
                                if (formScopeAtual.$id) {
                                    scopesFormVisitados.push(formScopeAtual.$id);
                                }
                                if (formScopeAtual[nomeFuncaoSemParenteses] && typeof formScopeAtual[nomeFuncaoSemParenteses] === 'function') {
                                    console.log('✅ [autoCompleta] Função encontrada no scope do formulário, nível', nivelForm);
                                    try {
                                        // Marcar função como executando
                                        funcoesExecutando[nomeFuncaoSemParenteses] = true;
                                        ...
                                        var resultado;
                                        if (parametros !== undefined) {
                                            resultado = formScopeAtual[nomeFuncaoSemParenteses].apply(formScopeAtual, Array.isArray(parametros) ? parametros : [parametros]);
                                        } else {
                                            resultado = formScopeAtual[nomeFuncaoSemParenteses].call(formScopeAtual);
                                        }
                                        ...
                                        console.error('❌ [autoCompleta] Erro ao executar função no scope do formulário:', e);
                                        funcoesExecutando[nomeFuncaoSemParenteses] = false;
                                    }
                                }
                                if (formScopeAtual.$parent) {
                                    formScopeAtual = formScopeAtual.$parent;
                                } else {
                                    break;
                                }
                            }
                ...
                // Estratégia 3: Procurar no elemento fora do modal
                try {
                    var elementoRaiz = elem;
                    var elementoForaModal = null;
                    while (elementoRaiz && elementoRaiz.length > 0 && elementoRaiz[0] !== document.body) {
                        if (elementoRaiz.closest('.modal, .popup-modal').length === 0) {
                            elementoForaModal = elementoRaiz;
                            break;
                        }
                        elementoRaiz = elementoRaiz.parent();
                    }
                    if (elementoForaModal && elementoForaModal.length > 0) {
                        var scopeElemento = angular.element(elementoForaModal[0]).scope();
                        if (scopeElemento && scopeElemento.$id !== scope.$id) {
                            console.log('🔍 [autoCompleta] Tentando procurar função no scope fora do modal');
                            var scopeElementoAtual = scopeElemento;
                            var nivelElemento = 0;
                            var scopesElementoVisitados = [];
                            while (scopeElementoAtual && nivelElemento < 15) {
                                nivelElemento++;
                                if (scopeElementoAtual.$id && scopesElementoVisitados.indexOf(scopeElementoAtual.$id) !== -1) {
                                    break;
                                }
                                if (scopeElementoAtual.$id) {
                                    scopesElementoVisitados.push(scopeElementoAtual.$id);
                                }
                                if (scopeElementoAtual[nomeFuncaoSemParenteses] && typeof scopeElementoAtual[nomeFuncaoSemParenteses] === 'function') {
                                    console.log('✅ [autoCompleta] Função encontrada no scope fora do modal, nível', nivelElemento);
                                    try {
                                        // Marcar função como executando
                                        funcoesExecutando[nomeFuncaoSemParenteses] = true;
                                        ...
                                        var resultado;
                                        if (parametros !== undefined) {
                                            resultado = scopeElementoAtual[nomeFuncaoSemParenteses].apply(scopeElementoAtual, Array.isArray(parametros) ? parametros : [parametros]);
                                        } else {
                                            resultado = scopeElementoAtual[nomeFuncaoSemParenteses].call(scopeElementoAtual);
                                        }
                                        ...
                                        console.error('❌ [autoCompleta] Erro ao executar função no scope fora do modal:', e);
                                        funcoesExecutando[nomeFuncaoSemParenteses] = false;
                                    }
                                }
                                if (scopeElementoAtual.$parent) {
                                    scopeElementoAtual = scopeElementoAtual.$parent;
                                } else {
                                    break;
                                }
                            }
                ...
                // Estratégia 4: Fallback - tentar usar $parse diretamente no scope original
                console.warn('⚠️ [autoCompleta] Função não encontrada nos scopes visitados. Tentando fallback com $parse...');
                try {
                    // Marcar função como executando
                    funcoesExecutando[nomeFuncaoSemParenteses] = true;
                    
                    var nomeCompleto = (nomeFuncao.indexOf('()') === nomeFuncao.length - 2) ? nomeFuncao : nomeFuncao + '()';
                    var parseFn = $parse(nomeCompleto);
                    var resultado = parseFn(scope);
                    ...
                    console.log('✅ [autoCompleta] Função executada via fallback $parse');
                    return resultado;
                } catch (parseError) {
                    console.error('❌ [autoCompleta] Não foi possível executar função:', nomeFuncao, parseError);
                    funcoesExecutando[nomeFuncaoSemParenteses] = false;
                    return null;
                }
            }
            ...
            var limparElemento = (dadosAC, modeloValor, modeloChave, modeloValor2, modeloValor3, modeloValor4) => {
                if (dadosAC.funcaoLimpar != undefined) {
                    // Obter o scope atual do elemento no momento da execução
                    var scopeAtual = elem.scope ? elem.scope() : (angular.element(elem[0]).scope ? angular.element(elem[0]).scope() : scope);
                    if (!scopeAtual) {
                        scopeAtual = scope;
                    }
                    // Usar a função auxiliar para encontrar e executar no scope correto
                    encontrarEExecutarFuncaoNoScope(dadosAC.funcaoLimpar, scopeAtual);
                }
            ...
            elem.bind("focus", function () {
                ...
                if (aC.limparNoFoco != undefined && (aC.limparNoFoco == 'true' || aC.limparNoFoco == true)) {
                    ...
                    if (aC.aoLimparNoFoco != undefined) {
                        // Obter o scope atual do elemento no momento da execução
                        var scopeAtual = angular.element(event.target).scope();
                        if (!scopeAtual) {
                            scopeAtual = scope;
                        }
                        // Usar a função auxiliar para encontrar e executar no scope correto
                        encontrarEExecutarFuncaoNoScope(aC.aoLimparNoFoco, scopeAtual);
                    }
                }
                ...
                jQuery.fn.autocompletaAngular = function (parametrosBusca) {
                    ...
                    $(this).autocomplete({
                        ...
                        select: function (event, ui) {
                            ...
                            // Encontrar o scope correto para atribuir valores
                            var scopeParaAtribuir = encontrarScopeParaAtribuir(modeloAC, event.target);
                            
                            $parse(modeloAC).assign(scopeParaAtribuir, ui.item.label);
                            ...
                            if (!eConsulta) {
                                ...
                                // Encontrar scope correto para modeloChaveAC também
                                var scopeChave = encontrarScopeParaAtribuir(modeloChaveAC, event.target);
                                $parse(modeloChaveAC).assign(scopeChave, ui.item.id);
                                ...
                                if (modeloValor2 != undefined) {
                                    ...
                                    var scopeValor2 = encontrarScopeParaAtribuir(modeloValor2, event.target);
                                    $parse(modeloValor2).assign(scopeValor2, ui.item.label2);
                                }
                                ...
                                if (modeloValor3 != undefined) {
                                    ...
                                    var scopeValor3 = encontrarScopeParaAtribuir(modeloValor3, event.target);
                                    $parse(modeloValor3).assign(scopeValor3, ui.item.label3);
                                }
                                ...
                                if (modeloValor4 != undefined) {
                                    ...
                                    var scopeValor4 = encontrarScopeParaAtribuir(modeloValor4, event.target);
                                    $parse(modeloValor4).assign(scopeValor4, ui.item.label4);
                                }

                                // Aplicar mudanças no scope correto ao invés do $rootScope
                                // Isso evita digest cycles desnecessários em toda a aplicação
                                try {
                                    if (scopeParaAtribuir && !scopeParaAtribuir.$$phase && !$rootScope.$$phase) {
                                        scopeParaAtribuir.$apply();
                                    }
                                } catch (e) {
                                    // Se falhar, tentar com $rootScope como fallback
                                    if (!$rootScope.$$phase) {
                                        $rootScope.$apply();
                                    }
                                }
                            ...
                            if (parametrosBusca.aoSelecionar != '') {
                                // Obter o scope atual do elemento no momento da execução
                                // Isso garante que usamos o scope correto mesmo após abrir/fechar modais
                                var scopeAtual = null;
                                try {
                                    // Tentar obter o scope do elemento atual
                                    var elementoAngular = angular.element(event.target);
                                    scopeAtual = elementoAngular.scope();
                                    // Se não conseguir, tentar obter do elemento isolado
                                    if (!scopeAtual) {
                                        scopeAtual = elementoAngular.isolateScope();
                                    }
                                } catch (e) {
                                    console.warn('⚠️ [autoCompleta] Erro ao obter scope do elemento:', e);
                                }
                                
                                // Se não conseguiu obter do elemento, usar o scope capturado
                                if (!scopeAtual) {
                                    scopeAtual = scope;
                                }
                                
                                console.log('🔍 [autoCompleta] Scope obtido para aoSelecionar. Scope ID:', scopeAtual ? scopeAtual.$id : 'null');
                                
                                // Usar a função auxiliar para encontrar e executar no scope correto
                                encontrarEExecutarFuncaoNoScope(parametrosBusca.aoSelecionar, scopeAtual);
                            }
