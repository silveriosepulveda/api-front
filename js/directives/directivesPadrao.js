var directivesPadrao = angular
    ...
    .directive("montaHtmlPosicao", [
        ...
        "APIServ",
        "EGFuncoes",
        function ($rootScope, $parse, $compile, APIServ, EGFuncoes) {
            return {
                restrict: "E",
                replace: true,
                transclude: false,
    ...
    .directive("selectFiltrosPesquisaOriginal", [
        "$compile",
        "$parse",
        "APIServ",
        "EGFuncoes",
                ...
                link: function (scope, elem, attr) {
                    var montaCampoValor = function (mascara, valor, indice, campo) {
                        ...
                        if (!angular.isObject(valor)) {
                            var input = `
                        <input type="text" ng-model="${modeloValor}" id="${idValor}" ${atributos.join(" ")} class="${classes.join(
                                " "
                            )}  form-control valorConsulta" ng-if="filtro.tipo != 'intervaloDatas'" placeholder="Defina o Valor">
                        ...
                        <div class="input-group" ng-if="filtro.tipo == 'intervaloDatas'">
                            ...
                            var input = `
                            <select ng-model="${modeloValor}" ng-options="key as value for (key, value) in filtro.valoresMascara" id="${idValor}" ${atributos.join(
                                " "
                            )} class="${classes.join(" ")} form-control">
                            </select>`;
                        }
                    ...
                    elem.bind(" change", function () {
                        ...
                        let dadosCampoFiltroPesquisa =
                            scope.estrutura.camposFiltroPesquisa != undefined && scope.estrutura.camposFiltroPesquisa[campo] != undefined
                                ? scope.estrutura.camposFiltroPesquisa[campo]
                                : {};
                        let dadosCampoFiltroPadrao =
                            scope.estrutura.filtrosPadrao != undefined && scope.estrutura.filtrosPadrao[campo] != undefined
                                ? scope.estrutura.filtrosPadrao[campo]
                                : {};
                        ...
                        scope.filtros[indice]["valor"] =
                            dadosFiltro["padrao"] != undefined ? dadosFiltro["padrao"] : dadosFiltro["valor"] != undefined ? dadosFiltro["valor"] : "";
    ...
    .directive("detalhesItemConsulta", [
        "$rootScope",
        "$compile",
        "APIServ",
        "EGFuncoes",
        "APIAjuFor",
        function ($rootScope, $compile, APIServ, EGFuncoes, APIAjuFor) {
            return {
                restrict: "E",
                replace: true,
    ...
    .directive("htmlDetalhe", [
        "$compile",
        "APIServ",
        "EGFuncoes",
        function ($compile, APIServ, EGFuncoes) {
            return {
                restrict: "E",
                replace: true,
                link: function (scope, elem) {
                    ...
                    if (mostrar) {
                        ...
                        var html = `                    
                    <div class="campoDetalheConsulta ${classesDiv.join(" ")}"  ${atributosDiv.join(
                            " "
                        )}>${label} <strong> {{item.detalhes.${campo}}}</strong></div>`;
    ...
    .directive("uiLista", [
        ...
        "APIServ",
        "EGFuncoes",
        function ($compile, $parse, APIServ, EGFuncoes) {
            return {
                restrict: "A",
                link: function (scope, elem) {
                    elem.bind("focus", function () {
                        ...
                        if (temBloco) {
                            if (dadosBloco.variavelSuperior != undefined) {
                                ...
                                var modeloChave =
                                    scope.estrutura.raizModelo +
                                    "." +
                                    dadosBloco.variavelSuperior +
                                    "[" +
                                    indice +
                                    ']["' +
                                    dadosBloco.nomeBloco +
                                    '"][' +
                                    indice2 +
                                    ']["' +
                                    dadosCampo.lista.campoChave +
                                    '"]';
                            } else {
                                if (dadosBloco.repeticao != undefined) {
                                    var modeloChave =
                                        scope.estrutura.raizModelo + "." + dadosBloco.nomeBloco + "[" + indice + ']["' + dadosCampo.lista.campoChave + '"]';
                                } else {
    ...
    .directive("validaFormulario", [
        "config",
        "$rootScope",
        "APIAjuFor",
        function (config, $rootScope, APIAjuFor) {
            return {
                require: "^form",
                priority: -1,
                link: function (scope, element, attr, form) {
                    element.bind("submit", function () {
                        ...
                        angular.forEach(form, function (value, key) {
                            ...
                            var elemento =
                                typeof value === "object" && value.hasOwnProperty("$modelValue") ? angular.element("[name='" + value.$name + "']") : undefined;

                            let temComparacoes =
                                elemento != undefined &&
                                (elemento.attr("igual") != undefined || elemento.attr("minimo") != undefined || elemento.attr("maximo") != undefined);
                            ...
                            APIServ.executaFuncaoClasse("classeGeral", "valorExiste", parametros).success(function (retorno) {
                                if (retorno.length > 0) {
                                    ...
                                    if (variavel_local == "" || scope[scope.estrutura.raizModelo][scope.estrutura.campo_chave] > 0) {
                                        //Neste caso vou apenas verificar se existe e informar
                                        ...
                                        var texto =
                                            campo_valor != undefined && campo_valor != ""
                                                ? "Valor Já Cadastrado para " + retorno[0][campo_valor]
                                                : "Valor Já Cadastrado";
                                        //manipularSpan(texto);
                                        APIAjuFor.manipularSpan(elem, elementoPai, texto, true);
                                    } else {
                                        //Neste caso vejo se existe, e se sim insiro o retorno no escopo
                                        ...
                                        if (temBloco) {
                                            ...
                                            if (dB.variavelSuperior != undefined) {
                                                ...
                                                variavel_local =
                                                    indice != undefined
                                                        ? `${scope.estrutura.raizModelo}['${dB.variavelSalvar}'][${indice}]`
                                                        : `${scope.estrutura.raizModelo}.${dB.variavelSalvar}`;
                                            }
                                        }
                                        console.log(retorno);
        ...
        return {
            ...
            link: function (scope, element, attr) {
                scope.$watch("carregando", function (val) {
                    if (val) APIServ.telaAguarde();
                    else APIServ.telaAguarde("fechar");
                });
            },
        };
    })
    // .directive("resumoConsulta", [
    //     "$compile",
    //     "EGFuncoes",
    ...
    .directive("inputBotao", [
        "$compile",
        "EGFuncoes",
        "APIServ",
        function ($compile, EGFuncoes, APIServ) {
            return {
                restrict: "E",
                ...
                link: function (scope, elem) {
                    ...
                    if (p.atributosButton != undefined) atributos = p.atributosButton;
                    else if (p.atributos_button != undefined) atributos = p.atributos_button;
    ...
    .directive("selectItemConsulta", [
        "$rootScope",
        "$compile",
        "APIServ",
        "EGFuncoes",
        "$parse",
        function ($rootScope, $compile, APIServ, EGFuncoes, $parse) {
            return {
                restrict: "E",
                replace: true,
                link: function (scope, elem, attr) {
    ...
    .factory("timerConsultaService", ($rootScope, $timeout) => {
        var timers = {};
        var TICK_MS = 100;

        function getOrCreate(classe, intervalo) {
            if (!timers[classe]) {
                var delay = Math.max(1000, (intervalo || 20) * 1000);
                var intervalId = null;
                var remaining = delay;
                var total = delay;
                var startTime = null;
                ...
                function updateBars(percent) {
                    barElements = barElements.filter(function(el) {
                        if (el && (el.isConnected !== false) && el.parentNode) {
                ...
                function fire() {
                    var escopo = $rootScope.estruturas && $rootScope.estruturas[classe];
                    if (escopo && escopo.filtrar) {
                        try {
                            if (!escopo.$$phase && !escopo.$root.$$phase) {
                                escopo.$apply(function() {
                                    escopo.filtrar(1, "timer");
                                });
                            } else {
                                escopo.filtrar(1, "timer");
                            }
                        } catch (e) {
                            console.error("timerConsulta: erro ao executar filtrar", e);
                        }
                    }
                    remaining = delay;
                    total = delay;
                ...
                function tick() {
                    ...
                    var percent = Math.max(0, (remaining / total) * 100);
                    updateBars(percent);
                    if (remaining <= 0) {
                        fire();
                    }
                ...
                timers[classe] = {
                    ...
                    pause: function() {
                        ...
                        remaining = total - (Date.now() - startTime);
                    ...
                    registerBar: function(el) {
                        if (el && barElements.indexOf(el) === -1) {
                            barElements.push(el);
                            el.style.width = (remaining / total * 100) + "%";
                        }
                    },
                    unregisterBar: function(el) {
                        barElements = barElements.filter(function(e) { return e !== el; });
                    },
                    reset: function() {
                        remaining = total;
                        startTime = Date.now();
                    ...
                    setPaused: function(val) { isPaused = val; }
                };
            }
            return timers[classe];
        }

        return {
            get: getOrCreate,
            destroy: function(classe) {
                if (timers[classe]) {
                    timers[classe].stop();
                    delete timers[classe];
                }
            }
        };
    })
    .directive("timerConsulta", ["$rootScope", "$timeout", "$route", "timerConsultaService", function($rootScope, $timeout, $route, timerConsultaService) {
        return {
            restrict: "E",
            template: `
                <div class="col-xs-12">
                    ...
                    <div class="barraProgressoTimer-container" style="width: 100%; margin-top: 10px;">
                        <div class="barraProgressoTimer barraProgressoTimerJs" style="width: 100%;"></div>
                    </div>
                </div>
            `,
            link: function(scope, elem, attr) {
                ...
                var classe = attr.classe || ($route.current && $route.current.classe) || scope.acao || "default";
                var progress = elem[0].querySelector(".barraProgressoTimerJs");
                if (!progress) return;

                var timer = timerConsultaService.get(classe, intervalo);
                timer.registerBar(progress);
                ...
                var initTimeoutId = $timeout(function() {
                    scope.iniciarTimer();
                }, 500);

                scope.$on("$destroy", function() {
                    timer.unregisterBar(progress);
                    if (initTimeoutId) {
