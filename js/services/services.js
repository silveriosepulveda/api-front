angular.module('servicos', ['ngMaterial', 'ngMessages'])
    ...
    if (typeof app !== 'undefined') {
        ...
        app.factory('APIServ', function ($rootScope, $http, config, $base64, $mdDialog, APIAjuFor) {
    ...
    angular.module('servicos', [])
        .factory('APIServ', function ($rootScope, $http, config, $base64, $mdDialog, APIAjuFor) {
            ...
            var _forceDialogAboveModal = function (isLoading) {
                ...
                var dialogZIndex = isLoading ? 100001 : 100000;      // Dialog base
                var containerZIndex = isLoading ? 100005 : 100002;   // Container SEMPRE mais alto
                var buttonZIndex = isLoading ? 100006 : 100003;      // Botões mais altos ainda
            ...
            var _mensagemSimples = function (titulo, texto, funcao, fecharModal = false) {
                ...
                // Apply z-index fix for modals
                _forceDialogAboveModal(false);

                return $mdDialog.show(dialogOptions).then(function () {
                    if (fecharModal) {
                        // Close the modal if specified
                        $mdDialog.hide();
                    }
                    if (funcao) {
                        funcao();
            ...
            var _filtraArray = function (array, chave, valor) {
                var retorno = [];
                angular.forEach(array, function (val, key) {
                    if (angular.isObject(val)) {
                        if (chave != '' && val[chave] == valor) {
                            retorno.push(val);
                        } else if (chave == '' && valor == '') {
                            retorno.push(val);
                        }
            ...
            return {
                ...
                mensagemSimples: _mensagemSimples,
                converteParametrosparaUrl: _converteParametrosparaUrl,
                filtraArray: _filtraArray,
                isInsideModal: _isInsideModal,
                forceDialogAboveModal: _forceDialogAboveModal
            };
        });
}
